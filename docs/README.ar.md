<p align="center">
  <img src="../src-tauri/icons/logo.png" alt="Handless logo" width="128" height="128" />
</p>

<h1 align="center">Handless</h1>

<p align="center">
  تحويل الكلام إلى نص على macOS، مجاني ومفتوح المصدر.<br/>
  اضغط اختصارًا، تحدّث، واحصل على النص في أي تطبيق. شغّله محليًا للخصوصية أو استخدم واجهات السحابة.
</p>

<p align="center">
  <a href="https://handless.elwin.cc"><img src="https://img.shields.io/badge/Website-handless.elwin.cc-ef6f2f" alt="Website" /></a>
  <a href="https://github.com/ElwinLiu/handless/actions/workflows/build-test.yml"><img src="https://github.com/ElwinLiu/handless/actions/workflows/build-test.yml/badge.svg" alt="Build" /></a>
  <a href="https://github.com/ElwinLiu/handless/actions/workflows/lint.yml"><img src="https://github.com/ElwinLiu/handless/actions/workflows/lint.yml/badge.svg" alt="Lint" /></a>
  <a href="../LICENSE"><img src="https://img.shields.io/badge/License-MIT-blue.svg" alt="License: MIT" /></a>
</p>

<p align="center">
  <a href="../README.md">English</a> ·
  <a href="README.zh.md">简体中文</a> ·
  <a href="README.zh-TW.md">繁體中文</a> ·
  <a href="README.ja.md">日本語</a> ·
  <a href="README.ko.md">한국어</a> ·
  <a href="README.es.md">Español</a> ·
  <a href="README.fr.md">Français</a> ·
  <a href="README.de.md">Deutsch</a> ·
  <a href="README.pt.md">Português</a> ·
  <a href="README.ru.md">Русский</a> ·
  <b>العربية</b> ·
  <a href="README.it.md">Italiano</a> ·
  <a href="README.tr.md">Türkçe</a> ·
  <a href="README.uk.md">Українська</a> ·
  <a href="README.vi.md">Tiếng Việt</a> ·
  <a href="README.pl.md">Polski</a> ·
  <a href="README.cs.md">Čeština</a>
</p>

## الميزات

- **النسخ المحلي** -- حمّل النماذج من الإعدادات، يعمل بالكامل على الجهاز
- **تحويل الكلام إلى نص سحابيًا** عبر OpenAI أو Soniox
- **كشف النشاط الصوتي** (النماذج المحلية فقط)
- **معالجة لاحقة بالذكاء الاصطناعي** لتنظيف النصوص المنسوخة أو إعادة تنسيقها
- **macOS** (Intel و Apple Silicon)
- **17 لغة**

## التثبيت

**[تحميل لنظام macOS](https://github.com/ElwinLiu/handless/releases/latest)** (Intel و Apple Silicon)

البناء من المصدر: راجع [BUILD.md](../BUILD.md).

## CLI

**التحكم عن بُعد** (يتواصل مع نسخة قيد التشغيل):

```bash
handless --toggle-transcription    # تبديل التسجيل
handless --toggle-post-process     # تبديل التسجيل + المعالجة اللاحقة
handless --cancel                  # إلغاء العملية الحالية
```

**خيارات بدء التشغيل:**

```bash
handless --start-hidden            # بدون نافذة رئيسية
handless --no-tray                 # بدون أيقونة شريط القوائم
handless --debug                   # تسجيل تفصيلي
handless --help                    # جميع الخيارات
```

ادمجها بحرية: `handless --start-hidden --no-tray`

> **macOS:** نفّذ الملف الثنائي مباشرة: `/Applications/Handless.app/Contents/MacOS/Handless --toggle-transcription`

## استكشاف الأخطاء وإصلاحها

`Cmd+Shift+D` يفتح لوحة التصحيح.

## المساهمة

راجع [CONTRIBUTING.md](../CONTRIBUTING.md). للترجمات: [CONTRIBUTING_TRANSLATIONS.md](../CONTRIBUTING_TRANSLATIONS.md).

## الرخصة

[MIT](../LICENSE)

## شكر وتقدير

مشتق من [Handy](https://github.com/cjpais/Handy) v0.7.8.

[Whisper](https://github.com/openai/whisper) | [whisper.cpp](https://github.com/ggerganov/whisper.cpp) | [NeMo Parakeet](https://github.com/NVIDIA/NeMo) | [Moonshine](https://github.com/usefulsensors/moonshine) | [SenseVoice](https://github.com/FunAudioLLM/SenseVoice) | [Silero VAD](https://github.com/snakers4/silero-vad) | [Tauri](https://tauri.app) | [Handy](https://github.com/cjpais/Handy)
